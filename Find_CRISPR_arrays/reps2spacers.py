...
genome_CRISPR_dict = { k : ['False'] for k in subprocess.run("grep -haoE '{}' {}*.nhr".format(args.regex_pattern, args.blast_db_path), shell=True, universal_newlines = True, capture_output=True).stdout.split('\n') if len(k) > 0} 
